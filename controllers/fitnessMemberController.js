// // Upgrade downgrade flter wala he
...
//     return { error: `${prefix}: start date is invalid.` };
...
//     return {
//       error: `${prefix}: discount (₹${discount}) cannot exceed plan fee (₹${planFee}).`,
//     };
//   }
//   if (finalAmount < 0) {
...
//     return {
//       error: `${prefix}: Cannot mark as Paid without feeType or activity.`,
//     };
//   }

//   if (af.plan && !validPlans.includes(af.plan)) {
//     return {
//       error: `${prefix}: invalid plan "${af.plan}". Must be one of: ${validPlans.join(", ")}.`,
//     };
//   }
...
//     return {
//       error: `${prefix}: invalid payment status "${af.paymentStatus}".`,
//     };
//   }
...
//     if (isNaN(pd.getTime()))
//       return { error: `${prefix}: payment date is invalid.` };
...
//   );
//   if (orphaned.length) {
//     await FeeAllotment.deleteMany({
//       _id: { $in: orphaned },
//       organizationId: orgId,
//     });
//     await FeePayment.deleteMany({
//       allotmentId: { $in: orphaned },
//       organizationId: orgId,
//     });
//   }
...
//       if (!allotment) {
//         throw new Error(
//           `CRITICAL: Paid fee without allotment for member ${member._id}`,
//         );
//       }
...
//   } catch (bookingErr) {
//     console.error(
//       "Recurring booking creation error (non-fatal):",
//       bookingErr.message,
//     );
//   }
// };
...
//       .populate("activityFees.activity", "name activityName")
//       .populate(
//         "activityFees.feeType",
//         "description annual halfYearly quarterly monthly weekly daily hourly",
//       )
//       .populate("activityFees.staff", "fullName name")
//       .select("-password");
...
//       if (req.file)
//         deleteOldPhoto(`/uploads/fitness/members/${req.file.filename}`);
...
//         .status(409)
//         .json({
//           message: `A member with mobile ${mobile.trim()} already exists.`,
//         });
//     }
...
//         return res
//           .status(400)
//           .json({ message: "Invalid activityFees format." });
//       }
//     }
...
//       return res
//         .status(400)
//         .json({ message: "Either activity or membership pass is required." });
//     }
...
//         .status(400)
//         .json({
//           message: "Duplicate activities are not allowed for the same member.",
//         });
//     }
...
//       return res
//         .status(409)
//         .json({ message: "A member with this mobile number already exists." });
//     }
...
//       return res
//         .status(400)
//         .json({ message: "At least one renewal entry is required." });
//     }
...
//         return res
//           .status(400)
//           .json({ message: `${prefix}: invalid activity ID.` });
//       }
//       if (r.feeTypeId && !isValidObjectId(r.feeTypeId)) {
//         return res
//           .status(400)
//           .json({ message: `${prefix}: invalid membership pass ID.` });
//       }
...
//         return res
//           .status(400)
//           .json({ message: `${prefix}: valid start date is required.` });
//       }
//       if (!r.endDate || isNaN(end.getTime())) {
//         return res
//           .status(400)
//           .json({ message: `${prefix}: valid end date is required.` });
//       }
//       if (end < start) {
//         return res
//           .status(400)
...
//         return res
//           .status(400)
//           .json({ message: `${prefix}: discount cannot exceed plan fee.` });
//       }
...
//       .populate(
//         "activityFees.feeType",
//         "description annual halfYearly quaterly monthly weekly daily hourly",
//       )
//       .populate("activityFees.staff", "fullName name")
//       .select("-password");
...
//     res
//       .status(500)
//       .json({ message: "Server error while renewing membership." });
//   }
// };
...
//       fieldErrors.push("Full name cannot be empty.");
//     if (mobile !== undefined) {
//       if (!mobile.trim()) {
...
//           .status(409)
//           .json({
//             message: `Another member with mobile ${mobile.trim()} already exists.`,
//           });
//       }
//     }
...
//       if (
//         (!Array.isArray(parsedActivityFees) ||
//           parsedActivityFees.length === 0) &&
//         !req.body.membershipPass
//       ) {
//         if (req.file)
//           deleteOldPhoto(`/uploads/fitness/members/${req.file.filename}`);
...
//               .status(400)
//               .json({
//                 message: `Activity ${i + 1}: a valid activity ID is required.`,
//               });
//           }
...
//             return res
//               .status(400)
//               .json({ message: `Activity ${i + 1}: fee type is required.` });
//           }
...
//             return res
//               .status(400)
//               .json({ message: `Activity ${i + 1}: invalid fee type ID.` });
//           }
//           // ──────────────────────────────────────────────────────────────
...
//             return res
//               .status(400)
//               .json({ message: `Activity ${i + 1}: invalid date range.` });
//           }
...
//       // Duplicate activity check
//       const activityIds = tempValidated
//         .map((af) => af.activity?.toString())
//         .filter(Boolean);
//       const uniqueActivityIds = new Set(activityIds);
//       if (uniqueActivityIds.size !== activityIds.length) {
...
//           .json({
//             message:
//               "Duplicate activities are not allowed for the same member.",
//           });
//       }
...
//     } catch (bookingErr) {
//       console.error(
//         "Slot booking error during update (non-fatal):",
//         bookingErr.message,
//       );
//     }
...
exports.getAllMembers = async (req, res) => {
  try {
    ...
    // const result = members.map((m) => applyComputedStatuses(m.toObject()));
    const result = members.map((m) => {
    return  obj = applyComputedStatuses(m.toObject());
    // return filterActivityFeesForResponse(obj);
  });
...
exports.renewMember = async (req, res) => {
  try {
    ...
    //   member.activityFees,
    // );
    for (const renewal of validatedRenewals) {

    const oldIndex = member.activityFees.findIndex(
      af =>
        af._id &&
        af._id.toString() === renewal._renewedFromId
    );

    if (oldIndex !== -1) {

      // move old membership to history
        member.historyFees.push(
          member.activityFees[oldIndex].toObject()
        );

      // remove old membership from active list
      member.activityFees.splice(oldIndex, 1);
    }

    delete renewal._renewedFromId;

    // add renewed membership as active
    member.activityFees.push(renewal);
  }
  ...
  member.membershipStatus = computeOverallMembershipStatus(
    ...
    try {
      const newFeesOnly = {
        _id: member._id,
        activityFees: validatedRenewals,
      };
      await syncFeesToTables(newFeesOnly, req.organizationId, []);
    } catch (syncErr) {
      console.error("Fee sync error during renewal:", syncErr.message);
