...
// // ─── Sync FitnessMember → User collection ─────────────────────────────────────
// // ← ADDED: Creates/updates a User record so member can login via authController
// const syncMemberToUser = async (member, hashedPasswordOrPlain = null) => {
//   try {
//     const existingUser = await User.findOne({ linkedId: member._id });

//     let finalPassword = existingUser?.password || null;

//     if (hashedPasswordOrPlain && hashedPasswordOrPlain.trim()) {
//       if (hashedPasswordOrPlain.startsWith('$2')) {
//         // Already hashed (post-save hook ran) — use directly
//         finalPassword = hashedPasswordOrPlain;
//       } else {
//         // Plain text — hash it
//         finalPassword = await bcrypt.hash(hashedPasswordOrPlain.trim(), 10);
//       }
//     }
...
//       isActive: member.status === 'Active' ? 'Yes' : 'No',
//       updatedAt: new Date(),
//     };

//     if (!existingUser) {
//       if (!finalPassword) {
//         console.error('syncMemberToUser: password missing for new user, skipping.');
//         return;
//       }
//       await User.create({ ...userData, password: finalPassword });
//     } else {
...
//     }
//   } catch (err) {
//     console.error('syncMemberToUser error (non-fatal):', err.message);
//   }
// };
...
//       address: address?.trim() || undefined,
//       userId: userId?.trim() || mobile.trim(),
//       password: password.trim(),  // plain text — FitnessMember pre('save') hook hashes it
//       enquiryId: enquiryId || null,
//       membershipPass: req.body.membershipPass || null,
//       activityFees: validatedFees,
//       membershipStatus: req.body.membershipPass
...
//     const member = new FitnessMember(memberData);
//     await member.save();  // ← pre('save') hook hashes password here

//     if (enquiryId) {
//       await FitnessEnquiry.findByIdAndUpdate(enquiryId, { status: 'Admitted' });
//     }

//     // ── Sync to User collection for login ─────────────────────────────────
//     // ← ADDED: member.password is now hashed after save, pass it directly
//     try {
//       await syncMemberToUser(member, member.password);
//     } catch (syncUserErr) {
//       console.error('User sync error (non-fatal):', syncUserErr.message);
//     }
...
//     let validatedFees = member.activityFees;
...
//     if (address !== undefined) member.address = address?.trim();
//     if (userId !== undefined) member.userId = userId?.trim();
//     if (password !== undefined && password.trim()) member.password = password.trim(); // hook rehashes
...
//     await member.save(); // ← pre('save') hashes password if changed

//     // ── Sync updated member to User collection ────────────────────────────
//     // ← ADDED: pass hashed password if it was changed, else null
//     try {
//       await syncMemberToUser(member, password?.trim() ? member.password : null);
//     } catch (syncUserErr) {
//       console.error('User sync error (non-fatal):', syncUserErr.message);
//     }
...
//     // ── Delete corresponding User entry ───────────────────────────────────
//     // ← ADDED: remove login access when member is deleted
//     try {
//       await User.findOneAndDelete({ linkedId: req.params.id });
//     } catch (delUserErr) {
//       console.error('User delete error (non-fatal):', delUserErr.message);
//     }
