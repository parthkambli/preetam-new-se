...
exports.updateActivity = async (req, res) => {
  try {
    ...
    // =====================================================================
    // FIX: Preserve existing slot _ids so bookings remain linked correctly.
    // If a slot comes in with an existing _id, keep it. If it's a brand-new
    // slot (no _id or unrecognised _id), Mongoose will generate a new one.
    // =====================================================================
    const activity = await FitnessActivity.findById(id);
    if (!activity)
      return res.status(404).json({ success: false, message: 'Activity not found' });
    ...
    activity.slots.forEach(s => {
      existingSlotMap[s._id.toString()] = s._id;
    });

    const mergedSlots = slots.map(slot => {
      const incomingId = slot._id?.toString();
      if (incomingId && existingSlotMap[incomingId]) {
        // Preserve the original ObjectId so all existing bookings still resolve
        return { ...slot, _id: existingSlotMap[incomingId] };
      }
      // New slot — let Mongoose assign a fresh _id
      const { _id, ...rest } = slot;
      return rest;
    });
...
exports.getBookings = async (req, res) => {
  try {
    ...
    if (activity?.trim()) {
      const matchedActivities = await FitnessActivity.find({
        name: {
          $regex: activity.trim(),
          $options: 'i'
        }
