...
// // ─────────────────────────────────────────────────────────────────────────────
// // ✅ Ensure upload directory exists at startup (Windows-safe)
...
//     const field = Object.keys(err.keyPattern)[0];
//     let friendlyMessage = `A record with this ${field} already exists.`;
//     if (field === 'mobile') friendlyMessage = 'This mobile number is already registered to another staff member.';
//     else if (field === 'loginId') friendlyMessage = 'This Login ID is already taken. Please use a different one.';
//     else if (field === 'email') friendlyMessage = 'This email address is already registered.';

//     return res.status(409).json({ success: false, message: friendlyMessage, field });
//   }

//   if (err.name === 'CastError') {
//     return res.status(400).json({ success: false, message: 'Invalid ID format. Please check the URL.' });
//   }
...
//   else if (data.fullName.trim().length < 2) errors.push('Full name must be at least 2 characters.');

//   if (!data.mobile?.trim()) errors.push('Mobile number is required.');
//   else if (!/^\d{10}$/.test(data.mobile.trim())) errors.push('Mobile number must be exactly 10 digits.');
...
//   if (data.salary !== undefined && data.salary !== '' && data.salary !== null) {
//     const salaryNum = Number(data.salary);
//     if (isNaN(salaryNum) || salaryNum < 0) errors.push('Salary must be a positive number.');
//   }
...
//     if (!staff) {
//       return res.status(404).json({ success: false, message: 'Staff member not found. They may have been deleted.' });
//     }
...
//       const staff = new Staff({
//         ...data,
//         photo: req.file ? `/uploads/staff/${req.file.filename}` : '',
//         organizationId: req.organizationId,
//         fullName: data.fullName.trim(),
//         mobile: data.mobile.trim(),
//       });
...
//     if (!staff) {
//       return res.status(404).json({ success: false, message: 'Staff member not found. They may have already been deleted.' });
//     }
...
//     res.json({ success: true, message: 'Staff member and their login account deleted successfully.' });
