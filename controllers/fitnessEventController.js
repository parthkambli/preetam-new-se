...
// Get all fitness events
// Get all fitness events (WITH PAGINATION + FILTERS)
exports.getAllEvents = async (req, res) => {
  try {
    const { title, type, date, page = 1, limit = 10 } = req.query;
    ...
    // 🔍 Date filter (FULL DAY RANGE — not exact match)
    if (date) {
    ...
    const total = await FitnessEvent.countDocuments(query);
    ...
    // ⚠️ RESPONSE STRUCTURE NOT CHANGED
    res.json({
      success: true,
      count: total, // total records (not just page count)
      data: events,
      pagination: {
        page: pageNum,
        limit: limitNum,
